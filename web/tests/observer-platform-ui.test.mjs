...
test("Observer Platform Capability aborts a stalled credential-free host request after seven seconds", async () => {
  const originalFetch = globalThis.fetch;
  ...
  globalThis.setTimeout = (callback, delay) => {
    ...
    const handle = Object.freeze({
      kind: "observer-platform-timeout",
  ...
  try {
    const first = apiLoad.module.fetchObserverPlatformCapability();
    const second = apiLoad.module.fetchObserverPlatformCapability();
    await Promise.all([
  ...
  for (const observedRequest of observedRequests) {
    assert.equal(observedRequest.path, "/v1/capabilities");
