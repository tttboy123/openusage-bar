...
test("Should-Send aborts a stalled credential-free host request after twelve seconds", async () => {
  const originalFetch = globalThis.fetch;
  ...
  try {
    const first = apiLoad.module.fetchShouldSendAdvice(request);
    const second = apiLoad.module.fetchShouldSendAdvice(request);
    await Promise.all([
  ...
  for (const observedRequest of observedRequests) {
    assert.equal(observedRequest.path, "/gateway/v1/should-send");
    assert.equal(observedRequest.init.credentials, "omit");
    assert.equal(
      observedRequest.init.headers?.["Content-Type"],
      "application/json",
    );
    assert.deepEqual(JSON.parse(observedRequest.init.body), request);
  }
  assert.deepEqual(scheduledDelays, [12_000, 12_000]);
...
test("Automation gates advice on capability state and cannot show stale results", () => {
  ...
  assert.match(
    pageSource,
    /adviceRequestPending\.current\s*\|\|\s*advicePhase\s*===\s*["']loading["']/,
    "a synchronous second submit must not start before React rerenders",
  );
  assert.match(pageSource, /adviceRequestPending\.current\s*=\s*true/);
  assert.match(pageSource, /adviceRequestPending\.current\s*=\s*false/);
  assert.match(
    pageSource,
