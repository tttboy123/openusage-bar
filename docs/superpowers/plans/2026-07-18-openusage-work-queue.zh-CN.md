...
- **WQ-27B：复用 Provider Center 推理凭证。** 在不把密钥交给 SwiftUI 的
  ...
  遗留旧 Activity 映像。随后完成 `0.8.2 (19) → 0.8.1 (18) → 0.8.2 (19)` 真实
  回滚与恢复：两次安装态均通过深度签名、Local API 和可见 Activity 重启检查，
  最终代理保持关闭，Provider 模板仍为 2 个且 JSON 不含密钥字段。演练以 TDD
  修复了回滚未关闭可见 Helper，以及签名包只读 Integrations 令成功回滚误报失败
  的两个发布缺口；随后把相同的安全清理边界覆盖到安装失败与卸载/数据清除路径。
  隔离发行 smoke 再次通过 clean install、upgrade、rollback、四个注入失败、保留
  数据和显式 purge，全程未触碰 Keychain；最终无 app 暂存目录残留。**
...
- **WQ-29：评测、故障注入与发行门禁。** 增加 replay/shadow 决策、性能基线、
  ...
  新令牌保持 200，原始令牌未落盘。`0.8.2 → 0.8.1 → 0.8.2` 的本机真实回滚
  演练也已完成。安全预审包 `docs/security/0.8-routing-review.md` 已按 STRIDE
  固化资产、信任边界、协议滥用测试、复现命令和初始五项残余风险；依赖审计无
  已知漏洞。预审发现的 DNS 校验/连接 TOCTOU 已用单次解析、全结果公网校验、
  数值 IP TCP 固定与原域名 TLS SNI/证书校验修复；仅允许在请求字节发出前切换
  到另一个已校验地址，执行重定向继续禁用。残余风险现为四项，源码/历史密钥与
  变更文档隐私扫描仍为 0。本机 `0.8.3 (20)` 已重新完成深度签名、事务安装、
  Local API、Activity 重启、代理默认关闭、2 个 Provider 模板无密钥输出与零
  安装暂存残留验收。该预审不替代独立批准；
  剩余工作仍是外部独立安全复核与公开 0.8 发布资格，完成前不得把 0.8 标记为
  公开发行。**
