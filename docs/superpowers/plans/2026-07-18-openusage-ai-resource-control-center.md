...
**Goal:** Turn OpenUsage Bar into a local-first macOS AI resource control center for people and a trustworthy read-only telemetry plane for Loom and other schedulers.

**Architecture:** Python remains the only owner of credentials, provider configuration, collection, and SQLite ledger writes. Native SwiftUI surfaces read sanitized facts; schedulers consume the same facts through a versioned Unix-socket API and CLI. Loom consumes observations through a separate adapter and remains the controller and state authority.
...
- Python owns configuration and ledger writes. SwiftUI and Loom are read-only consumers.
...
flowchart LR
    ...
    ledger --> api["Local API and CLI"]
    api --> loom["Loom Resource Observer"]
    loom --> policy["Loom policy and control plane"]
```
...
| Goal, policy, routing decision, SessionBinding, recovery | Loom |
| Execution lifecycle | Selected Harness under Loom governance |
| Predicted/reserved future usage | Loom |
...
| M3 Loom observer | 0.5.x | First typed API consumer, observe-only Loom adapter, and resource evidence | No scheduler behavior change; stable observation digest |
| M4 Release candidate to stable | 0.6.0 → 1.0.0 | Upgrade/rollback proof and 30-day external canary | All 1.0 gates in the release plan pass |
...
- [Loom resource observer](2026-07-18-openusage-loom-resource-observer.md)
...
Expected: the status command has no output and the tree command lists all six plans in this program. If the plans are not committed on the selected base, stop; do not create an implementation worktree from a commit that cannot contain its own instructions.
...
- [ ] Do not begin Loom integration until source-health and quota-history changes advance `dataRevision` and appear in `/v1/changes`.
...
### Task 5: Execute M3 Loom observe-only integration

**Files:**
- Follow: `docs/superpowers/plans/2026-07-18-openusage-loom-resource-observer.md`

- [ ] Use a separate clean Loom worktree; the currently inspected Loom worktree contains unrelated user changes.
- [ ] Keep the first adapter observe-only and disconnected from Scheduler and ProviderRouter.
- [ ] Bind observations into Session evidence only after SessionBindingController is present on the chosen Loom base branch.
- [ ] Require an explicit later authorization before any OpenUsage fact can influence active routing.

### Task 6: Execute release hardening and external canary
...
- Loom can produce a stable observation digest without reading SQLite, Keychain, or UI text.
