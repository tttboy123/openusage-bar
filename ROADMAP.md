...
| 0.8 Local Smart Routing | Turn resource facts and bounded Runtime observations into explainable Provider/account/model decisions, then add a separately enabled execution proxy. | **Phase A is complete in the local development build; public release integration remains in progress.** The deterministic engine, private stores, fact normalization, bounded evidence, second-socket Decision API, JSON CLI, explicit execution connections/targets, built-in and custom policies, master switch, Dry Run, explanations and bilingual native UI are implemented. Local development build 0.8.0 (10) passed accessibility, complete release gates and installed-app validation. Replay/shadow evaluation and the optional proxy have not shipped; the published baseline remains 0.6.0. | Content-free Decision API, deterministic reliability-first policy, Provider Center explanations and optional loopback proxy pass privacy, compatibility, performance, fault-injection and release gates without depending on Loom. |
