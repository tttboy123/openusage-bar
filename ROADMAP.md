...
| 0.8 Local Smart Routing | Turn resource facts and bounded Runtime observations into explainable Provider/account/model decisions, then add a separately enabled execution proxy. | **Specification accepted; WQ-26 in progress.** The deterministic engine, explicit private Route Target store and quota, native-currency balance and Runtime fact normalization are implemented locally. Decision evidence, API, UI and proxy have not shipped. | Content-free Decision API, deterministic reliability-first policy, Provider Center explanations and optional loopback proxy pass privacy, compatibility, performance, fault-injection and release gates without depending on Loom. |
