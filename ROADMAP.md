...
| 0.8 Local Smart Routing | Turn resource facts and bounded Runtime observations into explainable Provider/account/model decisions, then add a separately enabled execution proxy. | **Phase A and its local evaluation surfaces are complete; the optional proxy and public release remain open.** The deterministic engine, private stores, fact normalization, bounded evidence, second-socket Decision API, JSON CLI, explicit execution connections/targets, built-in and custom policies, master switch, Dry Run, explanations, Shadow/Replay evaluation and bilingual native UI are implemented. Local development build 0.8.0 (12) passed accessibility, complete release gates and installed-app validation; API reads now use an independent WAL connection so collection writes cannot hold the API object's lock. The published baseline remains 0.6.0. | Content-free Decision API, deterministic reliability-first policy, Provider Center explanations and optional loopback proxy pass privacy, compatibility, performance, fault-injection and release gates without depending on Loom. |
