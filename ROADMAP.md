...
| 0.8 Local Smart Routing | Turn resource facts and bounded Runtime observations into explainable Provider/account/model decisions, then add a separately enabled execution proxy. | **WQ-26 implemented locally; product integration remains in progress.** The deterministic engine, private Route Target store, fact normalization, bounded evidence store, strict second-socket Decision API and JSON CLI are implemented. Provider Center, execution adapters, proxy, replay evaluation and release gates have not shipped. | Content-free Decision API, deterministic reliability-first policy, Provider Center explanations and optional loopback proxy pass privacy, compatibility, performance, fault-injection and release gates without depending on Loom. |
