...
class DeepSeekOpenUsageAdapterTests(unittest.TestCase):
    ...
    def test_fetch_publishes_balance_fact_and_card(self):
        adapter = self._adapter()
        card = adapter.fetch()

        self.assertEqual(card.provider_id, "deepseek")
        self.assertIn("305.98", card.primary)
    ...
    def test_registry_builds_deepseek_binding_with_balance_and_cost_sources(self):
        ...
        self.assertEqual(len(deepseek.balance_sources), 2)
        self.assertEqual(len(deepseek.cost_sources), 1)
        self.assertTrue(hasattr(deepseek.balance_sources[1], "fetch"))
